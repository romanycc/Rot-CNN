...
def main():
    ...
    if args.train==True:
        ...
        if args.augment==True and args.dataset==1:
            model.load_state_dict(torch.load('./parameter/CNN2_MNIST_AUG.pt'))
        if args.augment==True and args.dataset==2:
            model.load_state_dict(torch.load('./parameter/CNN2_FashionMNIST_AUG.pt'))
        if args.augment==False and args.dataset==1:
            model.load_state_dict(torch.load('./parameter/CNN2_MNIST.pt'))
        if args.augment==False and args.dataset==2:
            model.load_state_dict(torch.load('./parameter/CNN2_FashionMNIST.pt'))
        print("【 Train set 】")
        test(model, device, train_loader)
        print("【 Test set 】")
        test(model, device, test_loader)
