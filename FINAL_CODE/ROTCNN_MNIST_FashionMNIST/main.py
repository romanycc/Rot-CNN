...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 32, 28, 1) 
        self.conv1.weight.data = flip_filter(self.conv1.weight.data) #flip filter
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        model.conv1.weight.grad = flip_filter(model.conv1.weight.grad) #flip filter
...
def main():
    ...
    if args.train==True:
        ...
        if args.dataset==1:
            model.load_state_dict(torch.load('./parameter/ROTCNN_MNIST.pt'))
        else:
            model.load_state_dict(torch.load('./parameter/ROTCNN_FashionMNIST.pt'))
        print("【 Train set 】")
        test(model, device, train_loader)
        print("【 Test set 】")
        test(model, device, test_loader)
